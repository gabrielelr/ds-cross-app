...
### R10 — Schede di documentazione dentro Figma SEMPRE in inglese

Quando compili frame di documentazione dentro al file Figma (`📱 Design System Cross-App`) — template `Purpose & Usage`, `Behavior`, e simili — il testo va scritto **sempre in inglese**, anche se la repo, i `metadata.json` e la conversazione con l'utente sono in italiano. *Why:* il file Figma è consultato anche da chi non parla italiano (team cross-app); le label dei template sono già in inglese e il source of truth condiviso deve essere coerente. L'italiano resta solo per i `metadata.json` (documentazione LLM-facing) e per la conversazione.

### R11 — Nessun `get_screenshot` di verifica dopo `use_figma`

Dopo aver scritto/modificato un frame con `use_figma`, **non chiamare `get_screenshot` per autoverificare** il risultato. L'utente ha Figma aperto e fornisce conferma o correzione direttamente. *Why:* lo screenshot di verifica brucia token e tempo senza aggiungere informazione utile; se `use_figma` non ha tornato errori, il risultato è già a schermo per l'utente. Eccezione: se `use_figma` ha riportato errore o nodi non trovati, allora un screenshot mirato serve a diagnosticare.

### R12 — Anti-pattern: Figma "di fila", JSON in parallelo

Quando riporti più di un anti-pattern dentro al template Figma `Purpose & Usage` (sezione `1.2 Anti-pattern (Don't ❌)`), **raggruppa per anti-pattern**: un blocco unico per ognuno con `Rule` + `Why it's wrong` + `Use instead` in sequenza (inline labels in bold), tutto dentro al text node "Rule" del template; **nascondi** (`visible = false`) i due frame paralleli "Why it's wrong" e "Use instead" per non lasciare label appese a contenuti vuoti. Sul `metadata.json` invece tieni la struttura parallela originale (`antiPatterns[]` con `scenario` / `reason` / `alternative` come campi separati per ogni oggetto). *Why:* su Figma il lettore umano segue meglio una narrazione completa per anti-pattern (vede in un colpo d'occhio cosa è sbagliato, perché, e come correggere); sul JSON la struttura parallela mantiene i tre aspetti machine-readable separati, utili a un LLM che fa pattern matching su `scenario` o `alternative` senza dover splittare un testo. Lo stesso principio si applica ad altri template Figma dove un template a 1 slot deve ospitare N item: collassa nel primo slot e nascondi i duplicati, non spalmare in parallelo.

### R13 — Se non applicabile, elimina; se applicabile, togli "(if applicable)"

Quando compili un frame di documentazione Figma e una sezione/sotto-riga **non è applicabile** al componente, **elimina (`visible = false`) l'intero paragrafo** invece di scrivere "None" / "N/A" / "Not applicable". Quando invece la sezione è applicabile, **rimuovi il suffisso `(if applicable)` / `(if available)`** dal titolo della sezione: con il contenuto compilato il suffisso è rumore. *Why:* "None" come contenuto sembra un valore intenzionale ma è informazione vuota che occupa spazio e fa sospettare al lettore che manchi documentazione; eliminare il paragrafo comunica chiaramente "questo componente non ha quella dimensione". Allo stesso modo, "(if applicable)" è un cue del template per il designer in fase di compilazione: una volta compilato non serve più. *How to apply:* per Badge Default → `Conditional logic` non c'è (componente puramente dichiarativo) → eliminare l'intera sezione; `Copy & truncation` esiste → togliere "(if applicable)". Per la sub-row `Interactive elements` di una Behavior section, se il componente è puramente display → eliminare la sub-row, non scrivere "None".

### R14 — Non duplicare nel Purpose info che vivono altrove (eccezione: size)

Nel `1.1 How and when to use it` (Do) NON ripetere informazioni che sono già documentate in un'altra sezione del frame Figma. In particolare le regole su label (lunghezza, troncamento, wrap, soglia di leggibilità) vivono in `Copy & truncation` del Behavior, NON nel Purpose. **Unica eccezione: le size**, che si menzionano sia in Purpose (perché hanno una connotazione di design: "md per surface grandi, sm per liste dense") sia in Behavior (con i numeri reali e i token). *Why:* duplicare l'informazione la mette fuori sync (uno dei due posti finisce per essere aggiornato e l'altro no), aumenta il rumore nel Purpose che dovrebbe restare scannabile, e confonde il ruolo delle sezioni — Purpose risponde a "quando e perché", Behavior risponde a "come si comporta in dettaglio". *How to apply:* nel Do tieni intro + varianti (cosa e quando) + sizes (scelta di design) + advisory cross-component (es. "per altre posizioni usa Badge Default"). Sposta in Copy & truncation tutto ciò che è sulla label (parole, caratteri, pill `whitespace-nowrap`, soglia 10pt).

### R15 — Bold solo su titoli e label di sotto-sezione, mai sul corpo

Quando applichi bold dentro un text node di documentazione Figma, **solo i titoli e le label di sotto-sezione vanno in bold**: il testo descrittivo che li segue resta in Regular. *Why:* il bold serve a creare scan landmark; se è applicato anche al corpo perde la sua funzione di indice visivo e il paragrafo diventa illeggibile. *How to apply:* prima di chiamare `setRangeFontName(Bold)` sui ranges target, **forza il font Regular sull'intero testo**: scrivi `node.characters = text`, poi `node.setRangeFontName(0, node.characters.length, { family, style: regularStyle })`, poi `setRangeFontName(Bold)` sui ranges dei titoli/label. Il "Regular style" va detectato per ogni family (candidati: `Regular`, `Medium`, `Book`, `Normal`). Senza il pass Regular preliminare, se il text node era originariamente in Bold il nuovo testo eredita Bold e il bold targettato risulta invisibile (tutto già grassetto).

### R16 — Confronti tra componenti vanno in una tabella (clonata dal template ufficiale)

Quando documenti un confronto tra più componenti (es. Toggle vs Checkbox vs Radio, Card Detail vs Card Informative vs Card Highlight, Alert vs Toast vs Banner), **NON usare una lista a bullet con nomi in bold**: crea una **tabella vera** dentro al Purpose Figma. *Why:* un confronto a 3+ righe e 2+ colonne (nome, quando applicarlo, use case) si scansiona di colpo in tabella ma fatica a leggersi in lista — l'occhio salta avanti e indietro fra etichetta e descrizione. La tabella mette ogni dimensione sulla stessa colonna per tutti i componenti, e si confronta in verticale a colpo d'occhio. *How to apply:* il template ufficiale di tabella nel file Figma è `9916:628` (header row 38pt + N data rows, 4 colonne con padding 10 e gap 26). **Clona quel frame** (`node.clone()`) invece di costruire da zero: preserva esattamente font, peso, padding, spacing e dimensioni del DS. Poi riscrivi solo i `characters` delle TEXT cell con i contenuti del confronto (walk del clone, raccogli i TEXT node in ordine, aggiorna per index). Insert il clone come sibling del body TEXT del Purpose, prima del frame "In which component / template / flow". Nel body TEXT tieni intro + closing che rimandano alla tabella ("see the table below").
